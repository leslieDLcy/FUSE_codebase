...
class Wind_load:
    ...
    def cp_wind_speed_interpolation(self, V_z):
        """ compute the average wind speed between two nodes for the elements
    ...
    def get_wind_speed_z(self, wind_model, turbulence_model):
        ...
        if wind_model == 'normal':
            V_z = self.V_hub * (self.hyperparameters.z /
                                self.hyperparameters.z_hub)**0.2
            ...
            V_z = self.hyperparameters.V_ref * \
                (self.hyperparameters.z / self.hyperparameters.z_hub)**0.11
            sample_z = V_z
        ...
        if turbulence_model == 'normal':
            ...
            sigma1 = 2 * self.hyperparameters.Iref * \
                (0.072 * (self.hyperparameters.V_ave/self.hyperparameters.c + 3)
                 * (self.V_hub/self.hyperparameters.c - 4) + 10)

        return sample_z, sigma1
    

    def get_wind_speed_z_interpolation(self, wind_model, turbulence_model):
        """ compute a sample of average wind speed for elements """
    ...
    def get_wind_speed_series(self, mu, sigma):
        ...
        ts = np.array([np.random.normal(mu, sigma, size=self.time * self.fs)
                      for mu, sigma in zip(mu, sigma_array)])
        return ts
    ...
    def plot_wind_speed_series(self, ts, node_index):
        ...
        ax.set_title('wind speed at height the {}m'.format(
            self.hyperparameters.z[node_index]))
    ...
    def get_wind_loads_F(self, wind_speed_series):
        ...
        linear_scale = 0.5 * self.hyperparameters.p * \
            self.hyperparameters.cd * self.hyperparameters.delta_h
        middle = [a * b**2 for a,
                  b in zip(self.hyperparameters.cross_sec_area, wind_speed_series)]
        result = [scalar * vector for scalar,
                  vector in zip(linear_scale, np.array(middle))]
    ...
    def cp_wind_loads_F_at_hub(self, ws_hub):
        """ V2: a more general compute the nodal force at the hub height

        args:
            ws_hub: wind speed at hub height;
        """
    ...
    def zero_padding(self, input_series, duration):
        """ Padding zeros to the end of the force series 

        args:
            duration: the length of the padding in seconds;

        note:
            padding to each column;
        """
        N = self.fs * duration
        if len(input_series.shape) == 1:
            npad = (0, int(N))
        elif len(input_series.shape) > 1:
            npad = ((0, 0), (0, int(N)))

        return np.pad(input_series, npad, 'constant')
    ...
    def plot_wind_loads(self, wind_loads, node_index):
        ...
        ax.set_title('drag forces at the height {}m'.format(
            self.hyperparameters.z[node_index]))
    ...
    def save_wind_load_series(self, ts, name, style='Francesca'):
        ...
        if style == 'Francesca':
            ...
            pass
        np.savetxt(f"data/{name}.txt", ts, delimiter=",", newline="\n")
    ...
    def cp_ws_elements(self,):
        """ For a more general way of computing average wind speed between nodes 
        
        note:
            1. for applying f on the elements;
            2. first compute the average height between two nodes;
            3. then compute the average wind speed between two nodes;
        """
        pass
